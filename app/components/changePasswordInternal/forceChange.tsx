...
export default function ChangePasswordModal({ onClose }: { onClose: () => void }) {
  ...
  const preventCopyPaste = useCallback((e: React.ClipboardEvent<HTMLInputElement> | React.ClipboardEvent<HTMLInputElement>) => {
    e.preventDefault();
    return false;
  }, []);
  ...
  const handleChange = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/users/${userId}/change-password`, {
        method: 'PUT',
    ...
    <div className="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4 transform transition-all">
        ...
        <div className="mt-6 flex flex-col space-y-3">
          ...
          <button
            onClick={onClose}
            className="w-full border border-gray-300 bg-white text-gray-700 py-2.5 px-4 rounded-lg font-medium hover:bg-gray-50 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
          >
            Cancel
          </button>
        </div>
