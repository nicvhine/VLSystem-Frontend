...
export default function LoanStatisticsCharts() {
  ...
  if (loading) return <p className="text-center py-8">Loading charts...</p>;
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      <div className="bg-white rounded-2xl p-4 shadow-lg border border-gray-200">
        <h3 className="mb-2 text-m font-semibold text-red-600">Loan Types</h3>
        <div className="relative w-full h-[220px]">
          <Pie data={loanTypeChartData} options={pieOptions} />
        </div>
        <div className="text-gray-600 mt-2 font-medium">
          Total Loans: {loanTypeStats.reduce((a, b) => a + b.count, 0)}
        </div>
      </div>

      <div className="bg-white rounded-2xl p-4 shadow-lg border border-gray-200">
        <h3 className="mb-2 text-m font-semibold text-red-600">Application Status</h3>
        <div className="relative w-full h-[220px]">
          <Pie data={applicationStatusChartData} options={pieOptions} />
        </div>
        <div className="text-gray-600 mt-2 font-medium">
          Total Applications: {applicationStatusStats.applied + applicationStatusStats.approved + applicationStatusStats.denied}
        </div>
      </div>
    </div>
  );
