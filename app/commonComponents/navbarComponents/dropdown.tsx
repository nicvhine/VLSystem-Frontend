...
export default function ProfileDropdown(props: ProfileDropdownProps) {
  ...
  return (
    <>
      ...
      {isDropdownOpen && (
        <div className="fixed inset-0 bg-black/50 z-[9998] md:hidden" onClick={closeDropdown}></div>
      )}
      ...
      >
        ...
        <div className="flex flex-col gap-0.5 px-6 pb-2">
          ...
          <div
            className={`transition-all duration-300 overflow-y-auto bg-gray-50 rounded-lg ${
              isEditing ? 'max-h-[50vh] opacity-100 mt-2' : 'max-h-0 opacity-0 mt-0 pointer-events-none'
            }`}
          >
      ...
      {/* Desktop Dropdown */}
      <div
        className={`hidden md:block bg-white text-gray-900 border border-gray-200 rounded-2xl shadow-2xl w-96 mt-3 p-0 mr-4 relative transition-all duration-300 ease-out transform
          ${isDropdownOpen ? 'opacity-100 scale-100 translate-y-0 pointer-events-auto' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'}`}
        style={{ 
          position: 'fixed',
          top: '4rem',
